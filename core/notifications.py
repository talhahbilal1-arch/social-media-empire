...
def _send_via_resend(subject: str, html_content: str, text_content: str) -> bool:
    """Try sending via Resend API (preferred — same as all other workflows)."""
    ...
    try:
        ...
        resp = requests.post(
            ...
            json={
                'from': 'System Alerts <alerts@updates.fitover35.com>',
                'to': [alert_email],
                'subject': subject,
                'html': html_content,
                'text': text_content,
        ...
        if resp.status_code < 300:
            ...
            return True
        print(f"Resend returned {resp.status_code}: {resp.text[:200]}")
        return False
...
def _send_via_smtp(subject: str, html_content: str, text_content: str) -> bool:
    ...
    try:
        ...
        print(f"Alert sent via Gmail SMTP: {subject}")
        return True
    except Exception as e:
...
def send_alert(subject: str,
    ...
    Tries Resend API first (consistent with all workflows), then falls back
    to Gmail SMTP. Returns True if sent successfully via either method.
    """
    full_subject = f"[{severity.upper()}] Social Media Empire: {subject}"
    now = datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')

    # Plain text version
    text_content = f"Social Media Content Empire Alert\n{'=' * 34}\n\n"
    text_content += f"Severity: {severity.upper()}\nTime: {now}\n\n{message}\n"
    if details:
    ...
    html_content = f"""
<div style="font-family:-apple-system,Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff;">
  <div style="background:{severity_color};color:white;padding:20px;">
    <h1 style="margin:0;font-size:20px;">Social Media Empire Alert</h1>
  </div>
  <div style="padding:20px;">
    <p style="color:#666;font-size:14px;margin-bottom:20px;">{now}</p>
    <div style="font-size:16px;line-height:1.6;margin-bottom:20px;">{message.replace(chr(10), '<br>')}</div>
    {"<div style='background:#f8f9fa;border-radius:4px;padding:15px;font-family:monospace;font-size:13px;white-space:pre-wrap;'>" + _format_details_html(details) + "</div>" if details else ""}
  </div>
  <div style="padding:15px 20px;background:#f8f9fa;font-size:12px;color:#666;">
    Automated alert from Social Media Content Empire.
  </div>
</div>"""

    # Try Resend first, fall back to Gmail SMTP
    if _send_via_resend(full_subject, html_content, text_content):
        return True
    if _send_via_smtp(full_subject, html_content, text_content):
        return True

    # Neither worked — log to console
