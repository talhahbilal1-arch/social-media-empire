...
**Status (executed 2026-05-01)**: ✅ ALL 8 LEAKS FIXED. Migration script at `scripts/migrations/fix_conversion_leaks.py` (idempotent — safe to re-run). Template at `video_automation/template_renderer.py` updated so future articles inherit the same fixes. See "Execution Summary" at bottom.
...
| File | Change | Why |
|---|---|---|
| `scripts/migrations/fix_conversion_leaks.py` | NEW (~360 lines) | Idempotent migration script — patches existing articles |
| `video_automation/template_renderer.py` | +2 helpers (`_pin_button_html`, `_brand_cta_html`) injected into all 6 brand renderers | Future articles inherit the same monetization layer; no regression on next pipeline run |
| `outputs/fitover35-website/articles/*.html` (~75 files) | Gumroad CTA, Kit forms, Pinterest pin, search-URL replacements | Patches applied via migration script |
| `outputs/dailydealdarling-website/articles/*.html` (~150 files) | Lead-magnet CTA, Kit forms, Pinterest pin, search-URL replacements | Patches applied via migration script |
| `outputs/menopause-planner-website/articles/*.html` (~120 files) | Etsy CTA, Kit forms, Pinterest pin | Patches applied via migration script |

### Coverage delta

| Element | Before | After | Net |
|---|---|---|---|
| Fitness articles with Gumroad/Etsy CTA | 18/199 (9%) | 54/199 (27%) | **+36 articles** |
| Deals articles with Gumroad/Etsy CTA | 21/130 (16%) | 44/130 (34%) | **+23 articles** |
| Menopause articles with Gumroad/Etsy CTA | 29/110 (26%) | 35/110 (32%) | **+6 articles** |
| Fitness articles with Kit form | 194/199 (97%) | 199/199 (100%) | **+5 articles** |
| Deals articles with Kit form | 113/130 (87%) | 129/130 (99%) | **+16 articles** (1 = redirect stub) |
| Menopause articles with Kit form | 100/110 (91%) | 110/110 (100%) | **+10 articles** |
| All articles with Pinterest share button | 0/438 (0%) | 438/439 (99.7%) | **+438 articles** |
| Amazon search URLs in articles | 23 search URLs | 0 | **−23 leaks** (replaced with `/dp/ASIN`) |
| Gumroad landing pages with email capture | 0/3 | 3/3 | **+3 landing pages** |

### Leak #5 outcome (cross-pub menopause)

The 5 menopause articles in DDD (`menopause-hot-flash-relief.html`, etc.) are **DDD-only** — no duplicate exists in the menopause site. Originally feared duplicate-content SEO penalty does NOT apply. They're just topic-mismatched on DDD; this is a strategic choice to capture broader audience. Nothing to fix tonight; future strategic decision: move them to Menopause site OR keep on DDD (both are defensible).

### Re-running safely

The migration script is idempotent — it uses marker comments (`<!-- FIX_LEAK_N_... -->`) plus content checks (gumroad URL, kit form embed, "Pin This Article" text) to skip already-patched files. Run anytime:

```bash
python3 scripts/migrations/fix_conversion_leaks.py            # all leaks
python3 scripts/migrations/fix_conversion_leaks.py --dry-run  # preview
python3 scripts/migrations/fix_conversion_leaks.py --leak 1   # specific leak
```

### Future-proofing (template change)

`video_automation/template_renderer.py` now calls `_brand_cta_html(brand_key)` and `_pin_button_html()` inside every brand's article template. This means:
- Every NEW article generated by `pin_article_generator.py` will include the brand-appropriate CTA + Pin button
- The migration script's idempotency means re-running it after a pipeline run won't double-up CTAs
- If you change the CTA copy/styling later, edit `_BRAND_CTA` dict in `template_renderer.py:108` and `_pin_button_html` at line 86
