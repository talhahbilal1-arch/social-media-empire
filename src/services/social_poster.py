...
class SocialPoster:
    ...
    def post_to_pinterest(
        ...
        Late API handles Pinterest's complex video upload workflow:
        - Registers media with Pinterest
        - Uploads video to Pinterest servers
        - Waits for video processing
        - Creates the pin with proper video format

        Args:
            video_url: Public URL of the video (must be accessible)
            metadata: Video metadata for the pin

        Returns:
            PostResult with success status and pin URL
        """
    ...
    def post_to_all_platforms(
        ...
        # First, upload to Supabase to get public URL (needed for Pinterest)
        video_url = None
