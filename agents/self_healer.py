...
class SelfHealer:
    ...
    # NOTE: content-brain, blog-factory, video-factory are archived.
    # The content-engine.yml now handles content generation + posting.
    AGENT_WORKFLOW_MAP = {
        'content_pipeline': 'content-engine.yml',
        'multi_platform_poster': 'content-engine.yml',
        'analytics_collector': 'analytics-collector.yml',
        'self_improve': 'self-improve.yml',
        'health_monitor': 'system-health.yml',
        'fitness_articles': 'fitness-articles.yml',
        'rescue_poster': 'rescue-poster.yml',
        'trend_discovery': 'daily-trend-scout.yml',
        'weekly_discovery': 'weekly-discovery.yml',
        'video_automation': 'video-automation-morning.yml',
        'tiktok_content': 'tiktok-content.yml',
    }
...
"""
    ...
    def _suggest_fix(self, agent_name: str) -> str:
        ...
        suggestions = {
            'content_pipeline': (
                "Check GEMINI_API_KEY is valid. Verify Pexels and Make.com webhook "
                "secrets. Re-run: gh workflow run content-engine.yml"
            ),
            'multi_platform_poster': (
                "Check Make.com webhooks are active. Verify LATE_API_KEY secrets. "
                "Re-run: gh workflow run content-engine.yml"
            ),
            'trend_discovery': (
                "Check GEMINI_API_KEY for trend analysis. Verify network connectivity. "
                "Re-run: gh workflow run daily-trend-scout.yml"
            ),
            'analytics_collector': (
            ...
            'health_monitor': (
                ...
                "SUPABASE_KEY. Re-run: gh workflow run system-health.yml"
            ),
            'self_improve': (
                "Check GEMINI_API_KEY. This agent runs weekly — verify the "
                "cron schedule. Re-run: gh workflow run self-improve.yml"
            ),
            'fitness_articles': (
                "Check GEMINI_API_KEY for article generation. "
                "Re-run: gh workflow run fitness-articles.yml"
            ),
            'rescue_poster': (
                "Check LATE_API_KEY and Pinterest board secrets. "
                "Re-run: gh workflow run rescue-poster.yml"
            ),
        }
