...
function generateFAQs(comparison, tool1, tool2) {
  return [
    ...
    { question: `Which has better ratings, ${tool1.name} or ${tool2.name}?`, answer: `Based on our expert testing, ${tool1.name} scores ${tool1.rating}/5 and ${tool2.name} scores ${tool2.rating}/5. ${tool1.rating > tool2.rating ? `${tool1.name} is rated higher.` : tool2.rating > tool1.rating ? `${tool2.name} is rated higher.` : 'Both are rated equally.'}` },
  ]
}
