...
def run_pipeline(
    ...
    for i in range(count):
        ...
        try:
            ...
            logger.info(f"{run_label} → Step 1: Generating script (format={video_format})...")
            script_data = generate_script(brand=brand, topic=topic, format=video_format)
...
""",
    ...
    parser.add_argument(
        "--format",
        default="standard",
        choices=["pinterest", "youtube", "standard"],
        help=(
            "Video format preset: pinterest=10-12s (1 body point, real video bg), "
            "youtube/standard=30-60s (3 body points). Default: standard"
        ),
    )
    parser.add_argument(
        "--topic",
